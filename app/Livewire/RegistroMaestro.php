...
{
    use WithPagination;
    public $torneo_id, $registro_id, $alumno_id, $competidor_id, $id;
    public $nombre, $apellidos, $email, $cinta, $peso, $estatura, $genero, $fec, $telefono, $usuario, $torneo, $torneo_datos, $tipoUsuario, $search, $edad, $costoTotal;
    public $step = 1;
    ...
    public function mount($torneo_id, $competidor_id = null)
    {
        ...
        if ($this->competidor_id){
            $usuario = Alumno::where('id', $competidor_id)
                            ->first();
                            $this->tipoUsuario = 'alumno';
            if (!$usuario) {
                $usuario = Maestro::where('id', $competidor_id)
                                ->first();
            ...
            $this->seleccionarCompetidor($competidorId, $nombre, $apellidos);
        }
    }
    ...
    public function seleccionarCompetidor($competidorId, $nombre, $apellidos)
    {
        /* dd($nombre, $apellidos); */
        $usuario = Alumno::where('id', $competidorId)
                            ->where('nombre', 'like', "%{$nombre}%")
                            ->where('apellidos', 'like', "%{$apellidos}%")
                            ->first();
                            $this->tipoUsuario = 'alumno';
        if (!$usuario) {
            $usuario = Maestro::where('id', $competidorId)
                            ->where('nombre', 'like', "%{$nombre}%")
                            ->where('apellidos', 'like', "%{$apellidos}%")
                            ->first();
                            $this->tipoUsuario = 'maestro';
    ...
    {
        if ($this->torneo_id && $this->tipoUsuario) {
            ...
            ->where(function ($query) {
                $query->where(function ($q) {
                    $q->whereNull('peso_minimo')
                        ->orWhere('peso_minimo', '<=', $this->peso);
                })
                ->where(function ($q) {
                    $q->whereNull('peso_maximo')
                        ->orWhere('peso_maximo', '>=', $this->peso);
                });
            })
            ->where(function ($query) {
    ...
    private function seleccionarCategoriaFusionadaAdecuada($fusion)
    {
        $categoriaAdecuada = $fusion->categorias()
            ...
            ->where(function ($query) {
                $query->where('cinta', $this->cinta) //  Buscar en la columna "cinta"
                    ->orWhere('cinta', 'LIKE', '%' . $this->cinta . '%') //  Buscar con "LIKE"
                    ->orWhere(function ($q) {
                        $q->where('cinta', 'P/I/A')
                            ->whereIn(DB::raw("'" . $this->cinta . "'"), ['principiante', 'intermedio', 'avanzada']);
                    })
            ...
            ->where(function ($query) {
                ...
                if ($this->mayor_de_edad) {
                    $categoriaAdecuada = $categoriaAdecuada->orWhere(function ($query) {
                        $query->where('edad_minima', '<=', 18)
                            ->where('edad_maxima', '>=', 18);
                    });
                }
            
                return $categoriaAdecuada->first();
    }
    ...
    public function incrementStep()
    {
        if ($this->step == 1) {
            ...
            if ($this->aceptoTerminos) {
                ...
                flash()->options([
                    'position' => 'top-center',
                ])->addError(' ', 'Debes aceptar ela responsiva para continuar.');
            }
    ...
    {
        if (is_array($this->selectedCategories) && count($this->selectedCategories) == 0 && $this->step == 4) {
            $this->step--;
            flash()->options([
                'position' => 'top-center',
        ...
        return view('livewire.registro-maestro', [
    ...
    private function completeRegistration()
    {
        ...
        foreach ($this->selectedCategories as $categoriaId) {
            ...
            if ($this->tipoUsuario === 'alumno') {
                $keys['alumno_id'] = $this->id;
            } else {
                $keys['maestro_id'] = $this->id;
            }
