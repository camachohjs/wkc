...
class RegistroAlumno extends Component
{
    ...
    public $torneo_id, $registro_id, $alumno_id;
    public $nombre, $apellidos, $email, $cinta, $peso, $estatura, $genero, $fec, $telefono, $usuario, $torneo, $torneo_datos, $tipoUsuario, $costoTotal;
    public $step = 1;
    ...
    public function mount($torneo_id)
    {
    ...
    {
        /* dd($value); */
        $this->buscarUsuarioPorCorreo($value);
    }
    ...
    public function buscarUsuarioPorCorreo($correo)
    {
        ...
        if (!$usuario) {
            $usuario = Maestro::where('email', $correo)->first();
            $tipoUsuario = 'maestro';
        }
    ...
    public function loadTorneoData()
    {
        if ($this->torneo_id && $this->usuario) {
            ...
            // Filtrar categorías
                /* dd($this->torneo_id, $this->usuario->edad, $this->usuario->peso, $this->usuario->cinta); */
    ...
    {
        if (count($this->selectedCategories) == 0 && $this->step == 3) {
            $this->step--;
            flash()->options([
                'position' => 'top-center',
        ...
        return view('livewire.registro-alumno', [
    ...
    {
        if ($stepOneData = session()->get('registro.stepOne')) {
            $this->nombre = $stepOneData['nombre'];
            $this->apellidos = $stepOneData['apellidos'];
            $this->email = $stepOneData['email'];
            $this->fec = $stepOneData['fec'];
            $this->cinta = $stepOneData['cinta'];
            $this->telefono = $stepOneData['telefono'];
            $this->peso = $stepOneData['peso'];
            $this->estatura = $stepOneData['estatura'];
            $this->genero = $stepOneData['genero'];
        }
        /* dd($stepOneData); */

        // Cargar datos del paso 2
        if ($stepTwoData = session()->get('registro.stepTwo')) {
            $this->selectedCategories = $stepTwoData['selectedCategories'];
        }
    }
    ...
    public function store()
    {
        if ($this->step == 1) {
            ...
            /* flash()->options([
                'position' => 'top-center',
            ])->addSuccess('Registro completado correctamente.');
    ...
    private function completeRegistration()
    {
        ...
        foreach ($this->selectedCategories as $categoriaId) {
            ...
            if ($this->tipoUsuario === 'alumno') {
                $keys['alumno_id'] = $this->usuario->id;
            } else {
                $keys['maestro_id'] = $this->usuario->id;
            }
