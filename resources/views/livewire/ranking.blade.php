<div class="px-5">
    <div class="d-flex align-items-center justify-content-between">
        ...
        <div class="text-right">
            <button class="btn btn-amarillo" wire:click="añadir"><i class="bi bi-person-add"></i> Añadir
                participante</button>
        </div>
    </div>
    <div class="d-flex">
        <div class="w-100 d-flex flex-column">
            <div class="w-100 rounded-lg bg-dark p-4"
                style="background: linear-gradient(145deg, #1c1c1c, #000000); border: 2px solid #EBC010; border-radius: 0.5rem; transition: transform 0.3s ease, box-shadow 0.3s ease;">
                <h5 class="text-white">
                    Filtros
                </h5><br>
                <div class="d-flex flex-column">
                    <div class="form-group w-100 mx-2">
                        ...
                        <input wire:model.live.debounce.150ms="competidor" type="text" class="form-control"
                            placeholder="Buscar Competidor..." id="competidor">
                    </div>
                    <div class="form-group w-100 mx-2 mt-4">
                        <label class="text-white" for="competencia">Competencia</label>
                        <select wire:model.live="competenciaSelected" wire:change="agregarCompetenciaBusqueda"
                            type="text" class="form-control" id="competencia" style="height: 100%;">
                    ...
                    {{-- <div class="form-group w-100 mx-2">
                        <label class="text-white" for="ciclo">Ciclo</label>
                        <select wire:model.lazy="cicloSeleccionado" class="form-control" id="ciclo">
                        ...
                        @foreach ($competenciasBusqueda as $competencia)
                            <div class="col-3 my-2">
                                <div class="position-relative">
                                    ...
                                    <button class="position-absolute top-0 end-0 btn btn-danger btn-sm m-1"
                                        style="border: none;"
                                        wire:click="eliminarCompetenciaBusqueda({{ $competencia->id }})">
                                        <i class="bi bi-x"></i>
                                    </button>
                                </div>
                            </div>
                ...
                <div class="form-group w-100 mx-2 mt-4">
                    ...
                    <div class="d-flex flex-nowrap overflow-auto">
                        <button wire:click.live="seleccionarTorneo('')"
                            class="btn mx-2 my-2 {{ $torneoSeleccionado === '' ? 'active' : '' }}"
                        ...
                        @foreach ($torneosLista as $torneo)
                            <button wire:click="seleccionarTorneo('{{ $torneo->nombre_torneo }}')"
                                class="btn mx-2 my-2 {{ $torneoSeleccionado === $torneo->nombre_torneo ? 'active' : '' }}"
                        ...
                        <div class="row">
                            <div class="col-11">
                                <span class="text-white h5">
                                    {{ optional($participantes->first())['division'] ?? 'Sin división' }}
                                </span>
                            </div>
                            <div class="col-1">
                                <i class="bi bi-chevron-down" wire:click="toggleCategoria({{ $categoriaId }})"
                            ...
                            <div class="mt-3">
                                ...
                                @foreach ($participantes as $participante)
                                    <div class="row mt-2 text-center">
                                        ...
                                        <div class="col-3"><img src="{{ $participante['foto'] }}" alt="foto_de_perfil"
                                                class="rounded-circle foto-perfil" style="height: 3rem; width: 3rem;">
                                        </div>
                                        ...
                                        <div class="col-1 px-2 d-flex flex-column text-center">
                                            <span class="text-white h5 mb-1"
                                                wire:click="mostrarDetalles({{ $participante['persona']->id ?? 'null' }})"
                                                style="cursor: pointer;">
                                                <i class="bi bi-chevron-down"></i>
                                            </span>
                                        </div>
                                        <div class="col-1 px-2 d-flex flex-column text-center">
                                            <span class="text-danger h5 mt-1"
                                                wire:click="eliminarParticipanteDelRanking({{ $participante['ranking_id'] }})"
                                                style="cursor: pointer;"
                                                wire:confirm="¿Estás seguro de eliminar al participantes del ranking?">
                                    ...
                                    @if ($participanteSeleccionado && $participanteSeleccionado->id == $participante['persona']->id)
                                        <div class="mt-3 p-3 rounded text-center">
                                            ...
                                            <p><img src="https://flagcdn.com/h24/{{ $participanteSeleccionado->codigo_bandera ?? 'unknown' }}.png"
                                                    alt="{{ $participanteSeleccionado->nacionalidad ?? '' }}"></p>
                                            <p><strong class="text-yellow">Edad:</strong> <span
                                                    class="text-white"></span>{{ $participanteSeleccionado->edad }}
                                            </p>
                                            <p><strong class="text-yellow">Cinta:</strong> <span
                                                    class="text-white"></span>{{ $participanteSeleccionado->cinta }}
                                            </p>
                                        </div>
                                    @endif
                                @endforeach
                        ...
                        @if (in_array($categoriaId, $categoriasAbiertas))
                            <div class="mt-3">
                                ...
                                @foreach ($participantes as $participante)
                                    <div class="row mt-2 text-center">
                                        <div class="col-1">{{ $participante['posicion'] }}</div>
                                        <div class="col-3"><img src="{{ $participante['foto'] }}"
                                                alt="foto_de_perfil" class="rounded-circle foto-perfil"
                                                style="height: 3rem; width: 3rem;"></div>
                                        <div class="col-4">{{ $participante['nombre'] }}</div>
                                        {{-- <div class="col-3">{{ $participante['pais'] }}</div> --}}
                                        <div class="col-2">{{ $participante['puntos'] }}</div>
                                        <div class="col-1 px-2 d-flex flex-column text-center">
                ...
                <div class="col-lg-12 my-2">
                    <div class="w-100 p-3 mt-3"
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <img src="{{ $participante['foto'] }}" alt="foto_de_perfil"
                                class="rounded-circle foto-perfil" style="height: 3rem; width: 3rem;">
                            <span class="text-white h5">
                                {{ $participante['nombre'] }} <img
                                    src="https://flagcdn.com/h24/{{ $participante['nacionalidad'] ?? 'unknown' }}.png"
                                    alt="{{ $participante['nacionalidad_nombre'] ?? '' }}">
                            </span>
                            <i class="bi bi-chevron-down" wire:click="togglePersona({{ $personaId }})"
                                style="cursor: pointer;"></i>
                        </div>
